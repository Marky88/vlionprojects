...
//    daySummaryData.show()
    ...
    daySummaryData.coalesce(2).write.mode("overwrite")
      .format("jdbc")
      .option("url", url)
      .option("user", user)
        .option("driver",driver)
...
//    newDayStatDF.createOrReplaceTempView("newDayStat")
    ...
    provinceSummaryData.coalesce(2).write.mode("overwrite")
      .format("jdbc")
      .option("url", url)
      .option("user", user)
        .option("driver",driver)
  ...
  def tagMap(spark:SparkSession): Unit ={
    val tagMapDF = spark.sql(
      """
        ...
        |""".stripMargin)
//    tagMapDF.show()
    tagMapDF.coalesce(2).write.mode("overwrite")
      .format("jdbc")
      .option("url", url)
      .option("user", user)
        .option("driver",driver)
  ...
  def incentivesSummaryByDay(spark:SparkSession,etlDate:String): Unit ={
    ...
    appStartFrame.coalesce(2).write.mode("append")
        .format("jdbc")
    ...
    val gameTimeFrame = spark.sql(
      s"""
         ...
         |) t
         |""".stripMargin)
    gameTimeFrame.coalesce(2).write.mode("append")
        .format("jdbc")
        .option("url", urlIncentives)
        .option("user", userIncentives)
        .option("driver",driver)
